function [HRC, PW, PH, PD, FL, FW, BW, rMagP, rPhi, rTheta, rAlpha] = validateTLEM2(Name)
...
rMagP = norm(meanPFP.HJF_pBW);

load('femoralCS_RM')

HJFtrans = meanPFP.HJF_pBW * orthoload2TLEM_RM;

rPhi   = atand(HJFtrans(3) / HJFtrans(2));
rTheta = atand(HJFtrans(1) / HJFtrans(2));
rAlpha = atand(HJFtrans(1) / HJFtrans(3));
