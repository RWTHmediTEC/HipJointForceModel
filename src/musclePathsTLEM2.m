...
for i = 1:length(MusclePaths)
    MusclePaths(i).MuscleListIdx = find(strcmp(MusclePaths(i).Name(1:end-1),MuscleList(:,1)));
    if isempty(MusclePaths(i).MuscleListIdx)
        warning('Muscle is not part of the muscle list of the cadaver!')
    else
        % Loop through the bones that are connected to the muscle
        Via = [];
        for b = MuscleList{MusclePaths(i).MuscleListIdx,3} 
...
if strcmp(MusclePathModel,'Wrapping')
    for i = 1:length(ActiveMuscles)
        ...
        for m = 1:length(mIdx) % loop through indices of active muscles in muscle list
                ...
                if size(MusclePaths(i).Points,1) <= 2 % Checks if there are no Via Points
                    for b = 1:2 % loop through bones with Surfaces
                        ...
                        for s = 1:length(Surface)
                            ...
                            if any(sBol)
                                % Initialize matGeom cylinder to test for a line cylinder intersection
